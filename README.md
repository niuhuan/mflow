...
如果你是新用户，或准备重新搭建工作流，推荐优先使用 [mflow-go](https://github.com/niuhuan/mflow-go)。可以直接下载，直接继承以前的配置文件。

`mflow-go` 的主要优势：

- 使用 Go + Wails + Vue3 重写，整体依赖更轻，构建和分发更直接。
- 前后端边界更清晰，配置、进程管理、游戏任务、版本检测等模块已拆分，后续维护成本更低。
- 支持范围更完整，除原有流程外，也补充了鸣潮等接入场景。
- 界面更好看
...
4. ok鸣潮出现错误后不会自动退出，直至到本程序设置的1小时超时后杀死程序。
...
- [三月七小助手FULL](https://github.com/moesnow/March7thAssistant/releases)
- [更好的原神](https://bettergi.com/download.html)
- [绝区零一条龙](https://github.com/OneDragon-Anything/ZenlessZoneZero-OneDragon/releases)
- [OK鸣潮](https://github.com/ok-oldking/ok-wuthering-waves/releases)
